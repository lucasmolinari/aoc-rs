...
impl<'a> Game<'a> {
    fn is_valid(&self, map: &HashMap<&str, i32>) -> bool {
        self.rounds.iter().all(|round| {
            round
                .iter()
                .all(|cube| cube.amount <= *map.get(cube.color).expect("Should be a valid Cube"))
        })
    }
}
...
fn game_parser(input: &str) -> IResult<&str, Game> {
    let (input, index) = preceded(tag("Game "), digit1)(input)?;
...
fn run(input: &str) -> i32 {
    ...
    games
        ...
        .filter_map(|game| {
            game.is_valid(&map)
                .then(|| game.index.parse::<i32>().expect("Should be a valid number"))
        })
        .sum::<i32>()
}

...
fn run(input: &str) -> u32 {
    ...
    let matches = cards
        .iter()
        .map(|card| card.matches() as usize)
        .collect::<Vec<usize>>();
    let store = (0..cards.len()).map(|i| (i, 1)).collect::<HashMap<usize, u32>>();
    matches.iter().enumerate().fold(store, |mut acc, (idx, matches)| {
        let add = *acc.get(&idx).expect("Should exist");
        println!("idx: {}, matches: {}, add: {}", idx, matches, add);
        for i in (idx + 1).. (idx + 1 + matches) {
            println!("idx: {} adding {} to {}", idx, add, i);
            acc.entry(i).and_modify(|e| *e += add);
        }
        acc
    })
    .values()
